...
namespace ContentSerialization.Controllers {
    public class AdminController : ApiController, IUpdateModel {
        ...
        // POST api/<controller>
        public IHttpActionResult Post(string id, JObject viewmodel) {
           if (! _contentDefinitionManager.ListTypeDefinitions().Any(d => d.Name == id)) {
               return BadRequest("Content definition does not exist.");
           }

            _model = viewmodel;

            var contentItem = _contentManager.New("Article");
